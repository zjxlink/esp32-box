...
static void wifi_scan()
{
    ...
    while (true) {
        ...
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to get WiFi scan count, error = %d", ret);
            break;
        }
        if (ap_count != 0) {
    ...
    ESP_LOGI(TAG, "Total APs scanned0 = %u, ret:%d", ap_count, ret);
    ESP_ERROR_CHECK(esp_wifi_scan_get_ap_records(&number, ap_info));
    ESP_LOGI(TAG, "Total APs scanned1 = %u", number);
    ...
    for (int i = 0; (i < DEFAULT_SCAN_LIST_SIZE) && (i < ap_count); i++) {
        ...
        /* 更新界面wifi*/
        gui_wifi_screen_add_ssid((char*)ap_info[i].ssid);
...
void wifi_reconnect_sta_figure(const char *ssid, const char *password)
{
    int bits = xEventGroupWaitBits(s_wifi_event_group, WIFI_CONNECTED_BIT, 0, 1, 0);
...
esp_err_t app_network_rescan(void)
{
    return send_network_event(NET_EVENT_SCAN);
}

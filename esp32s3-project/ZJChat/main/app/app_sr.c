...
static void audio_feed_task(void *arg)
{
    ...
    while (true) {
        if (FEED_DELETED && xEventGroupGetBits(g_sr_data->event_group)) {
        ...
        if (WIFI_STATUS_CONNECTED_OK == wifi_connected_already()) {
            ...
            ESP_LOGD(TAG, "feed, audio_chunksize=%d", audio_chunksize);
...
static void audio_detect_task(void *arg)
{
    ...
    while (true) {
        ...
        ESP_LOGD(TAG, "fetch data_size=%d", res->data_size / 2);
        audio_record_save2(res->data, res->data_size / 2);
...
esp_err_t app_sr_start(bool record_en)
{
    ...
    ret_val = xTaskCreatePinnedToCore(&sr_handler_task, "SR Handler Task", 10 * 1024, NULL, 6, &g_sr_data->handle_task, 1);
    ESP_GOTO_ON_FALSE(pdPASS == ret_val, ESP_FAIL, err, TAG,  "Failed create sr handler task");

    ret_val = xTaskCreatePinnedToCore(&player_handler_task, "SR Handler Task", 10 * 1024, NULL, 7, &g_sr_data->player_task, 1);
    ESP_GOTO_ON_FALSE(pdPASS == ret_val, ESP_FAIL, err, TAG,  "Failed create player handler task");
